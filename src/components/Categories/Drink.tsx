        ...
        <main>
            ...
            <Grid container  spacing={4}>
                <Grid item xs={12} sm={6} md={12}>
                    <Typography variant='h5'>Danh mục đồ uống</Typography>
                    <div>Hè nóng mát lạnh sảng khoái</div>
                </Grid>
                <Grid item xs={12} sm={6} md={12}>
                </Grid>
            </Grid>
                      ...
                      <div onClick={() => handleChangeDetailProduct(product.id)} className='product-card-content'>
                        ...
                        <CardContent sx={{ textTransform: "uppercase" }}>
                          <span style={{ fontSize: "19px", color: "#9a3a38", fontWeight: "600" }}>
                            {
                                (products.indexOf(product.id) !== -1) ? ((inforDiscount.discountType === 1 ? (priceFormat.format(Number(product.salePrice) - Math.floor(Number(product.salePrice) / 100 * Number(inforDiscount.discountValue)))
